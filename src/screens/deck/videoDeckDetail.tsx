...
import createDeckNoUrl from "../../assets/common/noImg/createDeckNoUrl.png"
...
import { getFirestore, addDoc, doc, getDoc, setDoc, updateDoc, collection,deleteDoc, onSnapshot} from 'firebase/firestore';
  ...
  const getDeckDatailData = (curUserData:any) => {
    ...
    if(isUserOnlyDeck)   {
      ...
      .then((deck:any) => {      
        ...
        setDeckDetailData(deck?.data());     
        console.log(deck?.data().savedCount,'deck?.data().savedCount')
        ...
        <ScrollView style={styles.contentContainer}>  
          ...
          <View style={[styles.deckImgContent,
           ...
           styles.premiumBorder
            ...
            {deckDetailData?.videoUrl ?
            <YoutubePlayer
                height={scaleX(203)}
                videoId={deckDetailData?.videoUrl}                
              />   
              : <Image source={createDeckNoUrl} style={styles.emtpyVideoImg} />
            }
          </View>
                ...
                maskElement={
                  ...
                  >
                    <Text style={styles.cardTypeTxt} >
                      {deckDetailData ? deckDetailData.deckType : 'Music123' }
                    </Text>
                  </View>
                }
              >
                <LinearGradient
                    colors={['#FFBE36', '#FFBE36']}
                    start={{ x: 0.45, y: 0 }}
                    end={{ x: 0.48 , y: 1 }}
                    style={{ flex: 1 }}
                  />
          ...
          <View style={[styles.rowContent, styles.marginTop27,styles.paddingStyle , {justifyContent: 'space-between'}]}>
            {/* <View style={[styles.rowContent, styles.tabbtnCotainer]}> */}
              ...
              {
                deckDetailData?.deckType == 'Music' &&
              <TouchableOpacity 
                onPress={() => WebBrowser.openBrowserAsync(deckDetailData?.spotifyUrl)} 
                >
          ...
          <View >
            {selectDescTab ?
            <View>
              <View style={[styles.rowContent, styles.marginTop27, styles.paddingStyle ,{justifyContent: 'space-between', alignItems: 'center'}]}>
                <TouchableOpacity style={[styles.rowContent, {alignItems: 'center'}]}
                  onPress = {()=>props.navigation.navigate(deckUserData?.userType ==='contentCreator' ? 'creatorProfile' : 'profile', { userId: deckDetailData?.userId })}>
                ...
                <View style={[styles.rowContent, styles.tabbtnCotainer, {justifyContent: 'flex-end', alignItems: 'center'}]}>
                  <TouchableOpacity 
                    onPress={() => props.navigation.navigate('createVideoDeck', {deckId, isUserOnlyDeck, userId})} 
                    style={[{alignItems: 'center',  marginRight: scaleX(10)}]}>
              ...
              <View style={styles.descTxtContainer}>
                {/* <Image source={textShapeBg} style={styles.descSahpeBg} /> */}
